    ...
    def _show_feedback(self, frame, c_frame, dict_maps, lower_hips_disp):
        ...
        for idx in np.where(c_frame)[0]:
            draw_text(
             ...
             # Check if well done feedback is required
            if dict_maps[idx][0] == "Well done":
                well_done_feedback = True

            # Speak the feedback
            text_to_speak = dict_maps[idx][0]
            engine.say(text_to_speak)
            engine.runAndWait()

        # If well done feedback is required, speak it
        if well_done_feedback:
